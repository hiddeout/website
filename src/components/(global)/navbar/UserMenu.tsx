        ...
        <>
            ...
            <div className="flex flex-row items-center justify-end">
                <div className="block lg:hidden">
                    <MdMenu
                        size={32}
                        className="hover:cursor-pointer hover:text-stmp-main"
                        onClick={() => setIsBurgerMenuOpen(!isBurgerMenuOpen)}
                    />
                </div>
                <Link
                    href="https://stmp.dev/discord"
                    className="hidden lg:flex items-center px-4 py-3.5 text-sm font-semibold transition duration-300 ease-linear bg-[#5865F2] hover:ring-2 hover:ring-[#5865F2] rounded-full">
                    <FaDiscord className="inline-block w-5 h-5" />
...
const BurgerMenu = ({ onClose }: { onClose: () => void }) => {
    ...
    const routes = useMemo(
        () => [
            ...
            {
                label: "Embeds",
                destination: "https://embeds.stmp.dev",
                isActive: pathname == "https://embeds.stmp.dev"
            },
            {
                label: "Docs",
        ...
        <>
            <div className="fixed inset-0 z-[9999999999] flex items-center justify-center">
                <motion.div
                    ...
                    className="bg-stmp-200 border -mt-40 border-stmp-card-border w-[90%] px-2 rounded-xl shadow-lg">
                    <div className="flex flex-row justify-between items-center gap-6 pt-10 px-4">
                        ...
                        <CgClose
                            size={24}
                            className="ml-auto hover:cursor-pointer hover:text-stmp-main"
                            onClick={onClose}
                        />
                    </div>
                        ...
                        {routes.map(route => {
                            return (
                                <Link
                                    ...
                                    key={route.label}
                                    onClick={onClose}
                                    className={`flex items-center h-14 bg-stmp-300 rounded-md ${
                                        route.isActive
                                            ? "text-stmp-main bg-stmp-200"
                                            : "text-stmp-700 hover:bg-stmp-dim hover:text-white"
                                    }`}>
