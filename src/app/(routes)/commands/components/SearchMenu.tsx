    ...
    return (
        <div className="fixed inset-0 z-[99] flex flex-col items-center pt-52">
            <motion.div
                ...
                ref={searchMenuRef}
                className={`bg-[#161717] border border-stmp-card-border z-[100] max-h-[30vh] w-[90%] sm:w-[600px] ${filteredCommands.length > 0 ? "rounded-t-2xl" : "rounded-2xl"}`}>
                <div className="flex flex-row w-full h-full items-center gap-2 p-4 text-[#616161]">
                    ...
                    <input
                        ...
                        className="text-white bg-transparent font-medium rounded-2xl p-2 focus:outline-none focus:border-stmp-pink w-full h-full"
            ...
            <div
                className={`flex flex-col border border-stmp-card-border overflow-hidden bg-stmp-600 gap-4w-[80vw] max-h-[32em] w-[90%] sm:w-[600px] ${filteredCommands.length > 0 ? "inline-block rounded-b-2xl" : "hidden"}`}
                ref={searchValuesRef}>
                <div className="flex flex-col gap-2 p-10 overflow-scroll overflow-x-hidden rounded-2xl -mt-6 bg-stmp-600">
                    {filter.map(category => (
                        <div key={category.name}>
                            <div className="flex flex-col gap-2 pt-2 pb-5">
                                {category.commands.map(command => (
                                    <div
                                        key={command.name}
                                        className="flex flex-col bg-stmp-400 rounded-2xl py-4 px-4 hover:cursor-pointer hover:bg-stmp-500"
                                        onClick={() => {
                                        ...
                                        <div className="flex flex-row items-center gap-2">
                                            <div className="text-stmp-700 text-sm">
                                                {category.icon}
                                            </div>
                                            <p className="text-stmp-700 font-medium text-sm">
                                                {category.name}
                                            </p>
                                        </div>
                                        <h1 className="text-white font-semibold">{command.name}</h1>
                                        <p className="text-sm font-normal text-stmp-secondary">
                                            {command.description}
                                        </p>
                                    </div>
                                ))}
