...
export const CommandsPage = ({
    ...
    // Set initial active category to the first available category
    useEffect(() => {
                    ...
                    <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        {activeCommands.map(command => (
                            <Command
                                ...
                                args={command.args.length > 0 ? command.args : ["None"]}
                                aliases={["None"]}
                                permissions={Array.isArray(command.permissions) ? command.permissions : [command.permissions ?? "None"]}
                            />
                        ))}
                    </div>
                </div>
...
const Command = ({
    ...
    return (
        <div
            id={name}
            className="flex flex-col py-6 rounded-3xl bg-stmp-200 border transition-shadow duration-200 ease-linear border-stmp-card-border text-white">
            <div className="h-full flex flex-col justify-between">
                ...
                <div>
                    <hr className="border-t border-stmp-card-border w-full my-4" />
                    <div className="px-6 flex flex-col gap-4">
                        <div>
                            <p className="text-sm tracking-wide text-stmp-main font-medium">
                                arguments
                            </p>
                            <div className="flex gap-2 mt-3 flex-wrap">
                                {args.length === 0 ? (
                                    <p className="text-[#D6D6D6] text-sm py-1 px-2 rounded-lg bg-stmp-300">
                                        None
                                    </p>
                                ) : (
                                    args.map((arg: string, index: number) => (
                                        <p
                                            key={index}
                                            className="text-[#D6D6D6] text-sm py-1 px-2 rounded-lg bg-stmp-300">
                                            {arg.replaceAll("_", " ")}
                                        </p>
                                    ))
                                )}
